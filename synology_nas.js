      ...
      // UPnP/1.0, Portable SDK for UPnP devices/1.6.18: https://github.com/pupnp/pupnp/commit/542c318acff73bf9be85b886a6e447bc473f57f2 
        ...
        this.findSimilarTitles = function (search_zero, search_one, search_two, videoItems) {
        	...
        	const similarities = videoItems.map(item => {
        		...
        		const distanceZero = this.levenshtein(cleanedTitle, search_zero);
			    const distanceZeroIndex = (cleanedTitle.indexOf(search_zero) > -1) ? 0 : 100; // если в названии есть поисковая строка, то сразу расстояние = 0
			    const distanceOne = this.levenshtein(cleanedTitle, search_one);
			    const distanceTwo = this.levenshtein(cleanedTitle, search_two);
