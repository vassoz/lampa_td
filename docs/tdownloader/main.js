...
.downloads-tab__meta-percent {
  display: inline;
}`;var f=`<svg xmlns="http://www.w3.org/2000/svg" width="34" height="34" viewBox="4 4 16 16">\r
...
.download-card__stats .eta {
  ...
  right: 0;
  font-size: inherit;
}`;function D(a,e){let t=$(Lampa.Template.get("download-card",w(a)));$(".full-start-new__right").append(t),t.on("hover:enter",()=>{y("full_start",a,e?.title||e?.original_title)}),t.on("hover:long",()=>{b("full_start",a,e?.title||e?.original_title)})}function K(a){let e=w(a),t=document.getElementById(`download-card-${e.id}`);if(t){for(let o in e){let n=t.querySelector(`[data-key="${o}"]`);n&&(n.textContent=e[o])}t.querySelector(".download-card__progress-bar-progress").setAttribute("style",`width: ${e.percent};`)}}function j(){Lampa.Template.add("download-card",W),$("body").append(`<style>${B}</style>`),Lampa.Listener.follow("full",a=>{if(a.type==="complite"){let e=l.getMovie(a.data.movie.id);e&&D(e,a.data.movie)}})}var Y=`<div class="download-circle d-updatable download-circle-{status}-{id}">\r
