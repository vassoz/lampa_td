"use strict";(()=>{var i={type:"other",version:"1.2.3",author:"https://github.com/kvart714",name:"Torrent Downloader",description:"Transmission RPC client",component:"t-downloader"};var O=i.component+".torrents.data.v2",c=class{static{this.data=Lampa.Storage.get(O,{torrents:[],info:{freeSpace:0}})}static getData(){return this.data}static getMovie(e){let t=this.data.torrents.filter(o=>o.id===e);return t.length>0?t.reduce((o,n)=>o.percentDone<n.percentDone?o:n):null}static ensureMovie(e){let t=this.data.torrents.filter(o=>o.externalId===e.externalId);return t.length>0?t.reduce((o,n)=>o.percentDone<n.percentDone?o:n):e}static async setData(e){this.data=e,Lampa.Storage.set(O,this.data)}};var z=`<div class="downloads-tab__list d-updatable">
...
`;var F=`<div class="downloads-tab__item selector {status}" data-id="{id}">
...
`;var U=`<li class="menu__item selector">\r
    <div class="menu__ico">{icon}</div>\r
    <div class="menu__text">{text}</div>\r
</li>\r
`;var M=`@keyframes pulseColor {
  0% {
...
</svg>`;var s={STOPPED:0,CHECK_PENDING:1,CHECKING:2,DOWNLOAD_PENDING:3,DOWNLOADING:4,SEED_PENDING:5,SEEDING:6,ISOLATED:7,STALLED:8,ERROR:9,ALLOCATING:10,MOVING:11,UNKNOWN:12,INITIALIZATION:13};function G(a){switch(a){case 0:return s.STOPPED;case 1:return s.CHECK_PENDING;case 2:return s.CHECKING;case 3:return s.DOWNLOAD_PENDING;case 4:return s.DOWNLOADING;case 5:return s.SEED_PENDING;case 6:return s.SEEDING;default:return s.UNKNOWN}}function q(a){switch(a){case"allocating":return s.ALLOCATING;case"checkingDL":case"checkingUP":case"checkingResumeData":return s.CHECKING;case"queuedDL":return s.DOWNLOAD_PENDING;case"queuedUP":return s.SEED_PENDING;case"downloading":case"forcedMetaDL":return s.DOWNLOADING;case"uploading":case"forcedUP":return s.SEEDING;case"pausedDL":case"pausedUP":case"stoppedDL":case"stoppedUP":return s.STOPPED;case"stalledDL":case"stalledUP":return s.STALLED;case"missingFiles":return s.ISOLATED;case"moving":return s.MOVING;case"error":return s.ERROR;case"metaDL":case"forcedDL":return s.INITIALIZATION;default:return s.UNKNOWN}}function h(a,e=2){if(a===0)return"0";let t=1024,o=e<0?0:e,n=Math.floor(Math.log(a)/Math.log(t));return parseFloat((a/Math.pow(t,n)).toFixed(o))+" "+Lampa.Lang.translate(`download-card.size.${n}`)}function we(a){let e=Lampa.Lang.translate("download-card.time.3");return`${h(a)}/${e}`}function ge(a){let e=Math.floor(a/86400),t=Math.floor(a%86400/3600),o=Math.floor(a%3600/60),n=Math.floor(a%60);return[e,t,o,n].map((p,f)=>p?p+Lampa.Lang.translate(`download-card.time.${f}`):null).filter(Boolean).slice(0,2).join(" ")}function w(a){return{id:a.id+"_"+a.externalId,fileName:a.status===s.INITIALIZATION?"Initialization":a.name,percent:(100*a.percentDone).toFixed(2)+"%",speed:a.speed>0?we(a.speed):"",downloadedSize:h(a.percentDone*a.totalSize),totalSize:h(a.totalSize),eta:a.status===s.DOWNLOADING?ge(a.eta):a.status===s.STALLED&&a.percentDone===1?Lampa.Lang.translate("download-card.status.14"):Lampa.Lang.translate(`download-card.status.${a.status}`),status:a.status===s.DOWNLOADING?"downloading":a.percentDone===1?"completed":"paused",seeders:`${a.seeders||0} (${a.activeSeeders||0})`}}function T(...a){console.log(i.name,...a)}var W=`<div class="selector download-card full-start__button d-updatable" id="download-card-{id}">\r
...
.download-card__stats .eta {
  ...
  right: 0;
  font-size: inherit;
}`;function _e(a,e){let t=$(Lampa.Template.get("download-card",w(a)));$(".full-start-new__right").append(t),t.on("hover:enter",()=>{y("full_start",a,e?.title||e?.original_title)}),t.on("hover:long",()=>{_("full_start",a,e?.title||e?.original_title)})}function K(a){let e=w(a),t=document.getElementById(`download-card-${e.id}`);if(t){for(let o in e){let n=t.querySelector(`[data-key="${o}"]`);n&&(n.textContent=e[o])}t.querySelector(".download-card__progress-bar-progress").setAttribute("style",`width: ${e.percent};`)}}function j(){Lampa.Template.add("download-card",W),$("body").append(`<style>${B}</style>`),Lampa.Listener.follow("full",a=>{if(a.type==="complite"){let e=c.getMovie(a.data.movie.id);e&&_e(e,a.data.movie)}})}var Y=`<div class="download-circle d-updatable download-circle-{status}-{id}">\r
